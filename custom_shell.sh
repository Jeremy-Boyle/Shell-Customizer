...
install_curl () {
    ...
    #verify command ran without errors
    if [ $? != 0 ]; then
    echo "Failed to install curl !! exiting" && exit
    else
        echo "Success !"
        sleep 2
        clear
...
install_oh_my_zsh () {
    ...
    command_exists git || {
        ...
        while true; do
            ...
            case $yn in
                [Yy]* ) sudo apt-get install git -y || echo -n "error" && echo "Installed"; break;;
                [Nn]* ) exit;;
                * ) echo "Please answer yes or no.";;
            esac
        done
...
EOF
    printf %s "$RESET"
    sed -i 's/(git)/zsh-syntax-highlighting zsh-completions git kubectl aws brew docker terraform vscode golang history/g' .zshrc
    ...
    while true; do
        clear
        read -p "Would like to install default theme settings? y/n: " yn
        case $yn in
            [Yy]* ) curl -o https://raw.githubusercontent.com/Jeremy-Boyle/Shell-Customizer/main/.p10k.zsh; break;;
            [Nn]* ) exit;;
            * ) echo "Please answer yes or no.";;
        esac
    done
